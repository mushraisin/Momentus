 ...
 * ── Рівні ──
 *
 * У всіх стартує перший. Ціна наступного дорівнює рівню, з якого йдемо:
 * 1→2 — 1 ✨FP, 2→3 — 2, і так до 14→15 — 14. Далі не дорожчає взагалі:
 * кожен рівень після пʼятнадцятого коштує рівно 15 ✨FP. Сам по собі рівень
 * нічого не дає — за нього відкривають плюшки, і оформлення профілю
...
/**
 * Стеля ціни одного рівня — вона ж рівень, на якому зростання спиняється.
 *
 * Спершу ціна подвоювалась: до десятого набігало 511 ✨FP, і рівні були
 * стіною замість поступу. Стеля це полагодила, але вперлась у неї вже
 * пʼятий рівень — далі всі коштували однаково, і поступ знову зникав.
 * Тепер ціна росте рівно на 1 за рівень і завмирає на пʼятнадцятому.
 */
...
export function levelCost(level) {
  ...
  // 1→2 = 1, 2→3 = 2 … 14→15 = 14, а з пʼятнадцятого — завжди стеля
  return Math.min(LEVEL_COST_CAP, from);
}
